...
class testClass(unittest.TestCase):
    ...
    def test_signup(self):
        print('Register test')
        resp=self.app.post('/register',data=dict(name='Madhu',mobileno='9845939375',lang='Kannada',email='Email_ID',password='abcdefg',confirm='abcdefg'),follow_redirects=True)
        #resp=self.app.get('/',follow_redirects=True)        
        self.assertEqual(resp.status_code,200)
        self.assertIn(b'Login',resp.data)
    def test_forgotpass(self):
        print('Forgot Password Test')
        resp=self.app.post('/forgotpass',data=dict(email='Email_ID'),follow_redirects=True)
        self.assertEqual(resp.status_code,200)
        self.assertIn(b'Password sent to the registered Email Address!',resp.data)
    def test_doc(self):
        print('Upload document test')
        resp=self.app.post('/login',data=dict(username='Karthik',password='abcdefg'),follow_redirects=True)
        resp=self.app.post('/upload/17/Karthik/emailid/7204250170/Kananda',data=dict(size='4',file='download2.jpg'),follow_redirects=True)
        self.assertEqual(resp.status_code,400)
        self.assertIn(b'Options',resp.data)
    def test_feedback(self):
        resp=self.app.post('/login',data=dict(username='Karthik',password='abcdefg'),follow_redirects=True)
        resp=self.app.post('/feedback/17/Karthik/emailid/7204250170/Kananda',data=dict(fd='goodwebsite'),follow_redirects=True)
        #resp=self.app.get('/portal',follow_redirects=True)                
        self.assertEqual(resp.status_code,200)           
